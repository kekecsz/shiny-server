...
bayes_plot_module_output <- function(id){
  ...
  box(title = "Correlation of height and shoe size",
      solidHeader = T,
      plotOutput(ns("plot")))
...
bayes_plot <- function(data, bf_min_participant){
  ...
  y_limit_max <- if_else(max(log(data$BF)) > log(310),
                         max(log(data$BF)),
                         log(310))
  
  y_limit_min <- if_else(min(log(data$BF)) < -log(310),
                         min(log(data$BF)),
                         -log(310))
  ...
  data %>% 
    ggplot() +
    aes(x = n_participant, y = log(as.numeric(BF))) +
    geom_point() +
    geom_line() +
    labs(y = "log(BF)", x = "Number of participants") +
    scale_y_continuous(breaks = c(c(-log(c(300, 30, 10, 3)), 0, log(c(3, 10, 30, 300)))),
                       labels = c("-log(300)", "-log(30)", "-log(10)", "-log(3)", "log(1)", "log(3)", "log(10)", "log(30)", "log(300)")) +
    scale_x_continuous(limits = c(bf_min_participant, x_limit_max)) +
    coord_cartesian(ylim=c(y_limit_min, y_limit_max)) +
    theme_minimal() +
    geom_hline(yintercept=c(c(-log(c(300, 30, 10, 3)), 0, log(c(3, 10, 30, 300)))), linetype="dotted", color="darkgrey") +
    geom_hline(yintercept=log(1), linetype="dashed", color="darkgreen") +
    geom_hline(yintercept=log(3), linetype="dashed", color="red") +
    geom_hline(yintercept=log(10), linetype="dashed", color="blue") +
    geom_hline(yintercept=log(30), linetype="dashed", color="darkgreen") +
    geom_hline(yintercept=-log(3), linetype="dashed", color="red") +
    geom_hline(yintercept=-log(10), linetype="dashed", color="blue") +
    geom_hline(yintercept=-log(30), linetype="dashed", color="darkgreen") +
    geom_point(data = data[nrow(data),], aes(x=n_participant, y=log(as.numeric(BF))), color="red", size=2) +
    annotate("text", x=x_limit_max, y=-4, label = "Very_strongH0", hjust=1, vjust=.5, size=3, color="black", parse=TRUE) +
    ...
    annotate("text", x=x_limit_max, y=4, label = "Very_strongH1", hjust=1, vjust=.5, size=3, color="black", parse=TRUE) +
